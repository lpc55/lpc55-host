...
pub fn app() -> clap::App<'static, 'static> {
    ...
    let app = App::new("lpc55")
        ...
        .subcommand(SubCommand::with_name("configure")
            .version(crate_version!())
            .long_version(LONG_VERSION.as_str())
            .about("configure factory and in-field settings")
            .setting(clap::AppSettings::SubcommandRequiredElseHelp)

            .subcommand(SubCommand::with_name("factory-settings")
                .version(crate_version!())
                .long_version(LONG_VERSION.as_str())
                .about("make changes to factory page (CMPA)")
                .arg(Arg::with_name("SETTINGS")
                     .help("filename of file containing settings")
                     .required(true)
                )
            )
            .subcommand(SubCommand::with_name("infield-settings")
                .version(crate_version!())
                .long_version(LONG_VERSION.as_str())
                .about("make changes to in-field page (CFPA)")
                .arg(Arg::with_name("SETTINGS")
                     .help("filename of file containing settings")
                     .required(true)
                )
